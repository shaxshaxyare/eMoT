...
const SignUp: React.FC<{ navigate: (view: View) => void; onSignUp: () => void; }> = ({ navigate, onSignUp }) => {
  return (
    <div className="min-h-screen bg-brand-gray-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md mx-auto bg-white p-8 rounded-2xl shadow-2xl border border-brand-gray-200">
        <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-brand-gray-900">Create Account</h1>
            <p className="text-brand-gray-600 mt-2">Join the Ministry Portal today.</p>
        </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSignUp(); }} className="space-y-4">
            <div className="relative">
                <User className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-brand-gray-400" />
                <input type="text" placeholder="Full Name" className="w-full border border-brand-gray-300 rounded-lg pl-10 pr-4 py-3 text-sm focus:ring-2 focus:ring-brand-blue-500 focus:border-brand-blue-500 outline-none" />
            </div>
             <div className="relative">
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-brand-gray-400" />
                <input type="email" placeholder="Email Address" className="w-full border border-brand-gray-300 rounded-lg pl-10 pr-4 py-3 text-sm focus:ring-2 focus:ring-brand-blue-500 focus:border-brand-blue-500 outline-none" />
            </div>
            <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-brand-gray-400" />
                <input type="password" placeholder="Create Password" className="w-full border border-brand-gray-300 rounded-lg pl-10 pr-4 py-3 text-sm focus:ring-2 focus:ring-brand-blue-500 focus:border-brand-blue-500 outline-none" />
            </div>
            <p className="text-xs text-brand-gray-500 !mt-2">By creating an account, you agree to our <a href="#" className="font-medium text-brand-blue-700 hover:underline">Terms of Service</a>.</p>
        ...
        <div className="mt-6 text-center text-sm">
            <p className="text-brand-gray-600">
                ...
                <button onClick={() => navigate('signin')} className="font-semibold text-brand-blue-700 hover:underline">Sign in</button>
            </p>
        </div>
      </div>
    </div>
